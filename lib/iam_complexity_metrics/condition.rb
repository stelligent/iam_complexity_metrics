...
class Condition
  ...
  def metric(statement_hash)
    return 0 unless statement_hash.has_key? 'Condition'

    aggregate = 0
    aggregate += statement_hash['Condition'].size * weights[:Condition]
    aggregate += confusing_value_operators(statement_hash['Condition'])
    aggregate += if_exists_operators(statement_hash['Condition'])
    aggregate += weights[:Null] if null_operator?(statement_hash['Condition'])
    aggregate += values_with_policy_tags(statement_hash['Condition'])
    aggregate
  end
